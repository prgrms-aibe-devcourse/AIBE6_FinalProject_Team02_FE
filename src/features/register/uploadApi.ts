...
/**
 * 클라이언트 1차 검증. 서버도 형식을 다시 보지만(§7), 사용자에게 즉시 알려 주려면 여기서도 본다.
 *
 * ⚠️ 용량은 presigned PUT 구조상 서버가 볼 수 없다 — 바이트가 서버를 거치지 않기 때문이다.
 * 등록 확정 시 서버가 S3 HeadObject로 실제 크기를 재확인해야 §7("클라이언트 검증만 믿지 않는다")이 성립한다.
 */
