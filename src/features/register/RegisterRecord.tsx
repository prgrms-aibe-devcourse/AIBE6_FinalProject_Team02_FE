...
import { AlertCircleIcon, ArrowLeftIcon, CheckIcon, ClockIcon, MapPinIcon } from 'lucide-react';
...
export function RegisterRecord({ submitting, error, onBack, onSubmit }: Props) {
  ...
  const draft = drafts[slot?.slotId] ?? { photoKeys: [], thumbnailKey: null, memo: '' };

  const patch = (change: Partial<Draft>) =>
    setDrafts((current) => ({ ...current, [slot.slotId]: { ...draft, ...change } }));
  ...
  const submit = () => {
    const cards: CardInput[] = recordSlots.map((passed) => {
      const saved = drafts[passed.slotId] ?? { photoKeys: [], thumbnailKey: null, memo: '' };
      return {
    ...
    onSubmit(cards, location.trim() ? { name: location.trim(), lat: null, lng: null } : null);
  };
  ...
  return (
    <div className="flex h-full flex-col bg-cream-100">
      ...
      <main className="no-scrollbar flex-1 overflow-y-auto px-5 pb-4">
        ...
        {/* AI가 확인하지 못한 칸은 검토를 거쳐야 열린다 — 기록 시점에 미리 알린다 (§5.2) */}
        {!slot.matched && (
        ...
        <section className="mt-4" aria-label="수집 위치">
          <label htmlFor="location" className="text-sm font-medium text-content-secondary">
            수집 위치 <span className="text-xs">(선택 · 모든 음식에 함께 적용돼요)</span>
          </label>
