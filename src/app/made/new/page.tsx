...
export default function MadeDexCreatePage() {
    ...
    const create = async (name: string, description: string, image: File | null, aiImageKey: string | null) => {
        // 이미지는 서버를 거치지 않고 S3로 직접 올린 뒤 key만 넘긴다. AI 결과면 이미 올라가 있다
        const imageKey =
            aiImageKey ?? (image ? (await uploadImageToS3(image, image.name || 'cover.jpg')).key : undefined)
    ...
    return (
        <MadeDexCreateWizard
            ...
            onIllustrate={() =>
                pushInApp(router, ROUTES.illustration({ purpose: 'LOGIT_COVER', returnTo: ROUTES.madeNew }))
            }
        />
    )
}
